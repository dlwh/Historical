...
\textit{\textipa{/ma:ti/}} and Malay \textit{/mata/}, both meaning
``eye.''~\cite{bloomfield38language} If we were to rely on
Levenshtein distance, these words would seem to be a highly attractive
...
\textit{\textipa{/i/}} in Greek wherever we see
\textit{\textipa{/a/}} in
...
latter task, we achieve F1 scores of 73.6\%. Both easily beat baseline
performance.
...
Here, the Italian word \textit{\textipa{/fwOko/}} (``fire'') is
generated from its parent form \textit{\textipa{/fokus/}} (``hearth'')
by a series of edits: two matches, two substitutions (\textit{/u/}$\to$
\textit{/o/}, and \textit{/o/}$\to$\textit{\textipa{/O/}}), one
...
consider the example of English ``day'' \textit{/\textipa{deI}/}
and Latin ``di\=es'' \textit{/\textipa{dIe:s}/}, both with the same
...
not undergo the sound change \textit{/\textipa{I}/}$ \to
$\textit{/\textipa{eI}/}.  Therefore, our model must learn not only
...
\newcite{bloomfield38language}. Handling these is a direction for
future research.}
...
deterministic automaton topology $\tilde\mu^*$, we create the
...
infer that Portuguese \textit{/d\textipa{1}femdu/}, Spanish
\textit{/defiendo/}, and Italian \textit{/dif\textipa{E}ndo/} are
all derived from Latin \textit{\textipa{/de:fendo:/}} ``defend.''
Using the simple Levenshtein transducers, on the other hand, our
model keeps all three separated, because the transducers cannot
know--among other things--that Portuguese \textit{/\textipa{1}/},
Spanish \textit{/e/}, and Italian \textit{/i/} are commonly substituted
for one another. Unfortunately, because the transducers used cannot
learn contextual rules, certain transformations can be over-applied.
For instance, Spanish \textit{\textipa{/nombRar/}} ``name'' is
grouped together with Portuguese \textit{/num\textipa{1RaR}/}
``number'' and Italian \textit{/numerare/} ``number,'' largely
because the rule Portuguese \textit{/u/} $\to$ Spanish \textit{/o/}
is applied outside of its normal context. This sound change occurs
primarily with final vowels, and does not usually occur word
medially. Thus, more sophisticated transducers could learn better
sound laws, which could translate into improved accuracy.
...
system of \newcite{bouchard09improved}. To evaluate, we matched each Latin word
...
reconstructed word and the chosen Latin word. As a kind of ``skyline,'' we

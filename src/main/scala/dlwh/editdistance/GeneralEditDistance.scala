...
import breeze.optimize.{CachedDiffFunction, FirstOrderMinimizer}
import dlwh.editdistance.EditDistanceObjectiveFunction._
...
class GeneralEditDistance(nStates:Int,
  ...
  def makeParameters[K](stats: Map[K, SufficientStatistic]):Map[K,Parameters] = {
    ...
    val init =  obj.featureEncoder.tabulateDenseVector(i => decodedParams(i))
    val params = FirstOrderMinimizer.OptParams(useStochastic = false, maxIterations = 20, regularization = 2).minimize(obj, init)

    for( (k,v) <- obj.featureEncoder.decode(params).iterator) {
      decodedParams(k) = v
    }
        ...
        if (i == 0 && j == 0) {
          ...
          if (j != 0) { // insertions
            ...
            while(state < nStates) {
              ...
              matrix(state)(LEFTEPS)(i,j) = logSum(scratch,nStates*2)
              state += 1
            }
          }
          if (i != 0) { // deletions
            ...
            while(state < nStates) {
              ...
              matrix(state)(RIGHTEPS)(i,j) = logSum(scratch,nStates*3)
              state += 1
            }
          }
          if(i != 0 && j != 0) { // matches
            ...
            while(state < nStates) {
              ...
              matrix(state)(NOEPS)(i,j) = logSum(scratch,nStates*3)
              state += 1
            }
          }
        }
      ...
      while (j >= 0) {
        if (i == s1.length && j == s2.length) {
          ...
          while(state < nStates) {
            ...
            matrix(state)(LEFTEPS)(i,j) = logSum(scratch,nStates*3)
            state += 1
          }
          // deletions
          // TODO: this could be much faster
          ...
          while(state < nStates) {
            ...
            matrix(state)(RIGHTEPS)(i,j) = logSum(scratch,nStates*2)
            state += 1
          }
